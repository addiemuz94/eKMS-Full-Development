...
/**
 * Step 4 Terminal Super Admin screen.
 *
 ...
 */
@Composable
fun KeyAccessAdminScreen(onBack: () -> Unit) {
    ...
    AlertDialog(
        ...
        text = {
            Column(
                ...
                Text("Fob UID scanning will be connected to protected Terminal hardware enrolment later. This form saves local demo metadata only.")
                OutlinedTextField(
                ...
                OutlinedTextField(
                    value = form.fobUid,
                    onValueChange = { form = form.copy(fobUid = it) },
                    modifier = Modifier.fillMaxWidth(),
                    label = { Text("Fob UID (optional)") },
                    isError = KeySlotAccessPolicy.errorFor(issues, KeyAccessField.KEY_FOB_UID) != null,
                    singleLine = true,
                )
                FieldError(KeySlotAccessPolicy.errorFor(issues, KeyAccessField.KEY_FOB_UID))
            }
        },
        confirmButton = {
            TextButton(
