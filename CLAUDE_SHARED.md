...
- **Gitignored Android secrets (Haikal, Aug 2026):** root `keystore.properties` (release signing for terminalApp + mobileApp) and `mobileApp/google-services.json` (Firebase/FCM) are never committed — ask Adi. **Debug builds and Android Studio sync do not need them**; release assemble/bundle still fails clearly without `keystore.properties`, and FCM is skipped until `google-services.json` is present. Details: `CLAUDE_TERMINAL.md` / `CLAUDE_MOBILE.md` Known issues.
