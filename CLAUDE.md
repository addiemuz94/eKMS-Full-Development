...
| [CLAUDE_WEB.md](CLAUDE_WEB.md) | `web/`-specific: Registration (Location wizard), Cabinet Management (Location→cabinet→settings; Assign User / Keys / Key Permission / Key Access tab), standalone **Key Access** queue (`/key-access`), User Management, Dashboard Overview, Sign out + Website settings, session-expiry → login, login email+password only, Deleted items / Erase data, `web/` Project Status |
...
4. **All mutations are revision-safe.** Update/PATCH-style requests carry `expectedRevision`; the backend rejects stale writes with `409 CONFLICT` rather than silently overwriting (verified server-side: every PATCH route checks `existing.revision === expectedRevision` AND guards the UPDATE itself with `WHERE revision = :expectedRevision`, a real double-checked guard against races, not just an application-level check). The frozen `webApp` never had in-place edit/PATCH UI. `web/` now does, for 10 resources (Units, Terminals, Personnel, Keys, Permissions, Event Setup, Schedules, User Groups, Key Groups, Multi-Authentication Rules — see `CLAUDE_WEB.md` Project Status) — every edit path reads `expectedRevision` off the already-loaded row and shows an explicit conflict message on `409` rather than retrying or overwriting. Appointments remain create/delete-only by backend design (only review/permissions-patch are mutable). **Key slots** are create/update/soft-delete via `/v1/admin/key-slots` and are managed from Registration / Cabinet Management → Keys (layout Rows × Columns + per-slot key assign/edit/delete).
5. **Delete is soft-delete by default.** Records move to a Super Admin-only Recycle Bin for 60 days (`RecycleBinPolicy.RETENTION_DAYS`) before purge; active dependents must block a **hidden** cascade delete. Two audited Super-Admin opt-in exceptions: (a) **cabinet cascade** — `DELETE /v1/admin/terminals/:id` with body `{ cascade: true }` soft-deletes the cabinet plus its slots, keys bound to those slots, and grants referencing those keys into the Recycle Bin (portal always sends cascade); (b) **Flush data** — `POST /v1/admin/flush` permanently hard-deletes by scope (never Super Admin / GOD_ADMIN users; audit history is kept). Historic audit events also survive normal Recycle Bin purge.
