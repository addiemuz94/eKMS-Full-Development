...
- **Fingerprint and face enrollment are fully coded and building but not hardware-verified** — no R503 module or camera was available this session. Fingerprint is protocol-tested via `FakeSerialTransport` only; face has zero live runs of the camera/liveness/capture pipeline. Verify on a real F7G18P before treating either as working.
- **Face enrollment ships RGB-only active liveness (blink + head-turn), a deliberate, user-confirmed v1 choice** — weaker anti-spoof than the vendor manual's RGB+IR spec (section 4.8.3/4.8.4, not available in this repo). Explicit plan to upgrade later; do not treat this as final or silently "fix" it without revisiting the tradeoff.
- **`terminalApp/src/main/assets/models/` adds ~41 MB of binary face-model assets to the repo** (no Git LFS configured) — be aware of this before further asset-heavy additions.
...
### Next steps (in order)
- Verify fingerprint enrollment (`FingerprintEnrollmentScreen`/`FingerprintHardwareController`) and face enrollment (`FaceEnrollmentScreen`/`FaceCameraController`) on a real F7G18P — first physical run of both
