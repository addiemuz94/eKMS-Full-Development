...
- **Auto-launch on boot, startup self-diagnostic, network bring-up.** Three related pieces, terminalApp-only. Added `BootCompletedReceiver` (registered on `android.intent.action.BOOT_COMPLETED`, launches `MainActivity` with `FLAG_ACTIVITY_NEW_TASK`) plus the `RECEIVE_BOOT_COMPLETED`/`ACCESS_NETWORK_STATE`/`ACCESS_WIFI_STATE` manifest permissions. New `hardware/NetworkStatusController.kt` observes connectivity via `ConnectivityManager` (deliberately not `android.net.EthernetManager`, which needs system-level permissions most OEM images won't grant a regular app) and prefers Ethernet over Wi-Fi when both are present; Wi-Fi bring-up is a best-effort `WifiManager.reconnect()` nudge only — modern Android gives a regular app no way to force-connect a specific saved SSID, so this relies on the OS's own standard auto-reconnect, per an explicit "no new pairing UI" instruction. New `ui/StartupDiagnosticsScreen.kt` runs automatically on every cold launch/crash-recovery relaunch (gated via `rememberSaveable`, which resets on a genuine process restart but not on background/foreground) — one line per subsystem (cabinet, fingerprint, camera presence, public NFC reader, network), reusing `HintSeverity` (extended with a new `FAIL` case) and `ConnectionHint` rather than a parallel severity system. Camera check is presence-only (`CameraManager.cameraIdList` contains `FaceCameraController.RGB_CAMERA_ID`) — not a full `FaceCameraController` instantiation, which would load OpenCV/MediaPipe and open the camera on every single boot. The NFC line reuses the same `PublicCardReaderController` instance `TerminalAdminApp` already auto-starts at idle-login rather than opening a second owner of `/dev/ttyS2` (confirmed `TerminalNfcReaderController` and `PublicM1CardReader` both claim that same port exclusively — running both would conflict). The screen actively triggers `hardwareController.connect()`/`fingerprintHardwareController.connect()` if not already connected — a real behavior change (these serial ports now open proactively at every launch instead of lazily on first real action), needed so "status" means something on a cold launch rather than "never tried" looking identical to "broken."
  **Verified this session — hardware-confirmed, not just compile/assemble:** restarted the app on a real F7G18P multiple times via `adb shell am force-stop` + relaunch; the diagnostics screen appeared each time with all five checks resolving live (Cabinet `/dev/ttyS1 @ 19200` OK, Fingerprint `/dev/ttyS0 @ 57600` OK, Camera OK, NFC reader OK, Network OK via Wi-Fi), Continue correctly dismissed to the normal login screen without disturbing the existing NFC auto-start, and a simple background/foreground cycle (home button, relaunch) correctly did *not* re-show it. **Not verified — cannot be verified without an actual reboot:** the boot receiver itself. Only confirmed by reading the manifest/receiver code, never by power-cycling the device. Also untested: the Ethernet-preferred path (this device only ever had Wi-Fi available during testing) and the fully-offline degraded state (didn't toggle Wi-Fi off, to avoid disrupting the device's live adb connection or its use for other testing).

- **Sound & voice line audit (no code changed).** Asked to prepare `AudioFeedbackController`'s 4 raw sound files for drop-in replacement and wire any genuinely-existing-but-silent flows. Confirmed by grep that no other file references `beep.wav`/`please_take_the_key.wav`/`please_insert_the_key.wav`/`please_close_the_door.wav` beside `AudioFeedbackController.kt` itself — replacing file bytes in place at the same filename is genuinely all that's needed, nothing else caches or bundles them. Checked the two flows named as candidates for new sounds (a wrong-slot return warning, a continuous multi-key return "Done" session) against the actual code: neither exists yet — `CabinetHardwareController.pollForKeyInsertion` only ever polls the one node the caller already identified, with no cross-node wrong-slot detection anywhere, and `TerminalKeyReturnScreen` has no session/continuous-return concept at all. Per instruction not to invent call sites for unbuilt flows, added nothing — no new `VoiceLine` entries, no new raw resources, no placeholders. Also checked every stage transition in both Take/Return flow screens for a silently-missing voice line where a sibling transition has one; found none (the absence of a completion/failure chime is already consistent across both flows, not a gap).

- **Key Menu & multi-key sequential Take Flow** — see "Terminal App UX Baseline (Production)" enhancement #3 above for the behavior spec. `CabinetHardwareController.beginKeyTake` was refactored into a shared `beginKeyTakeInternal` plus a new `beginQueuedKeyTake` (adds the red-off "your turn" transition before the same engage/eject/confirm sequence) and a new `beginMultiKeyRedLightSequence` (per-node `redLightOn`, one worker-thread operation, no batch command, no keep-alive). New `ui/TerminalKeyMenuScreen.kt` filters keys by the signed-in user's `AccessGrant.keyIds` (respecting a validity window if set) and reuses `TerminalKeyRetrievalScreen`'s `KeyDisplayMode`/`SoftSegmented` Layout/List toggle (promoted from `private` to `internal` for that reuse) rather than building a new toggle. `TerminalAdminApp` gained a `SuperAdminRoute.KEY_MENU`, a `MultiKeyTakeQueue` state class, and redirects all three Technician/Vendor post-login sites from the old unfiltered `KEY_RETRIEVAL` to `KEY_MENU` — Super Admin's own dashboard-reached `KEY_RETRIEVAL` path is untouched. The queue renders the *same, unmodified* `TerminalKeyTakeScreen` once per node (just re-keyed as the queue advances), so Take Warning Time / door-left-open logging / abandonment apply per-node with no parallel implementation; the next node's take is only ever started from the current node's `onCompleted`, by which point `takeMonitoring` has already been released, so two nodes can never be mid-take at once by construction, not by an added check.
  **Spec/code discrepancy found:** the task description attributed the one-electromagnet-at-a-time guard to `CabinetHardwareController.kt`; it actually lives in `shared/.../protocol/KeyCabinetLink.kt` (`engagedNodeAddress`/`ElectromagnetConcurrencyException`, section 10.4) — `CabinetHardwareController` only adds its own `takeMonitoring` app-level guard on top. Both layers are untouched by this change.
  **Verified: compile/assemble only.** `:terminalApp:build` (compile, lint, debug+release assemble) passes clean. **Not hardware-verified, and not live-verified at all beyond a regression check:** the real backend (`kms-cvt.com`) this device is paired to has exactly one personnel record (the Super Admin) and zero access grants, so there is no Technician/Vendor account to sign in as and reach `KEY_MENU` through the real login flow. Creating a throwaway test technician + grant to actually exercise it was offered and **declined** — leave this untested until that account exists. The only live check performed was a regression check: the Super Admin's existing unfiltered `KEY_RETRIEVAL` grid (reached from the dashboard) still works correctly after the `beginKeyTake` refactor. Completely unverified: `TerminalKeyMenuScreen`'s actual render (multi-select, Layout/List toggle, the "no keys assigned" empty state), the red-light batch sequence, the red→blue transition, and multi-node queue advancement — none of this has run against real hardware or a real account.

- **Return Flow rework** — see "Terminal App UX Baseline (Production)" §2's "Return Flow rework additions" above for the behavior spec. New `data/TerminalCheckoutStore.kt` (explicit Phase 5 stand-in — keyId/userId/terminalId/takenAtEpochMillis only, no overdue/emergency/extension, no backend sync); opened in `handleTakeFlowOutcome`'s `Success` branch (covers both single-key Take Flow and the Key Menu's multi-key queue, since both already funnel through that one handler), closed in `handleReturnFlowOutcome`'s `Success` branch. `CabinetHardwareController.pollForKeyInsertion` gained a wrong-slot sweep (initially `testMicroSwitch`-based presence-only, see the follow-up entry below for the revision to identity-based) over every other configured `KeySlot` node, with red-light/warning/forced-beep callbacks; `TerminalKeyReturnScreen` wired those in on top of its existing beep-loop pattern. `ReturnFlow` (in `TerminalAdminApp.kt`) gained a `SessionIdle` case for the continuous multi-key session (new `ui/ReturnSessionScreen.kt` for its Done-button screen) and an `attemptId: Long` (`System.nanoTime()`) field on `AwaitingCertification`/`DoorOpen`.
  **Duplicate-abandonment bug — believe this is now structurally fixed, not just hoped-fixed.** Root cause confirmed by tracing the old code: the abandonment-race `LaunchedEffect` was keyed on the *entire* `ReturnFlow` value, so a failed certification-login retry (same attempt, only `loginError` differs) produced a structurally-different value and cancel-relaunched the coroutine for no functional reason — and left a real TOCTOU gap where an already-completed `delay()` and a fresh login result could both decide to log the same event. Fixed two ways, both required: (1) keyed on `attemptId` alone, stable across login retries for the same swipe; (2) re-reads live state and confirms it's still the *same* `attemptId` immediately before logging/clearing — closes the race by construction, not by narrowing a window. A real design bug was also caught and fixed *during this same session*, before it shipped: the first draft tried to recover the continuous session's "returned this session" list by reading `returnFlow as? SessionIdle` inside the completion handler, but by that point `returnFlow` already held the just-finished attempt's own state, not the session that preceded it — the list would never have accumulated. Fixed by tracking session totals (`returnSessionStartedAtEpochMillis`/`returnSessionReturnedKeyNames`) in dedicated state independent of whichever `ReturnFlow` sub-state is currently active.
  **Beep-continuity bug — explicitly NOT fixed, and not believed fixable by this kind of change.** CLAUDE.md's existing description attributes this to `SoundPool`/`MediaPlayer` interaction on the device's custom `awplayer` audio stack, inside `AudioFeedbackController` — a class this rework never touches (Take Flow uses the identical pattern and was correctly left alone, out of scope). A state-machine rework cannot fix an audio-driver-level interaction; reporting this as resolved would have been exactly the "assume the rewrite fixes it" mistake the task explicitly warned against.
  **Verified: compile/assemble only** (`:terminalApp:build` passes clean). **Not hardware-verified at all** — wrong-slot detection and the continuous-session Done/timeout behavior specifically need a real cabinet with a spare key to mean anything; none of this has run on a device.

- **Wrong-slot detection revised: identity via `testMicroSwitchAndCard`, not bare presence via `testMicroSwitch`** (targeted follow-up to the Return Flow rework above, not a new phase). Verified byte semantics against `docs/Key Cabinet Communication Protocol.md` §5.4 rather than assuming: for command 0x17, a readable 4-byte value is a card UID, all-`0x00` is "bolt present, no card," all-`0xFF` is "nothing present" — the same encoding `waitForReturnedKeyFob` already relied on elsewhere in this file, reused rather than re-derived. `pollForKeyInsertion` gained a `resolveKeyFobUid: (rawUid: String) -> Boolean` callback (called from the controller's own worker thread, not main — must stay a fast, side-effect-free lookup) and `onWrongSlotDetected` gained a `confirmedKey: Boolean` flag. `TerminalAdminApp.resolveKeyFobUid()` checks **both** `personnelCardStore` and `keyCardStore` through `CardUidResolver`, returning true only for `CardUidMatch.Key` — per the permanent NFC UID Resolution Rule, never assumed from context (a personnel card physically sitting in a key slot is unlikely but not impossible, and must not be misreported as a key). **Deliberate design choice, not an oversight:** unresolved presence (no card, or a UID matching nothing enrolled) still triggers the same red-light/warning/beep alarm as a confirmed key — differentiated only by wording ("Wrong key" vs. "Unrecognized item") — because a real enrolled key should always carry a readable card, so anything else occupying an unexpected node mid-return is exactly as anomalous as a confirmed wrong key; erring toward flagging it was judged safer than silently ignoring it.
  **This makes the sweep-timing caveat below more of a concern, not less** — 0x17 does everything 0x16 did plus a card read, so every candidate node in the sweep now does strictly more work per cycle than the original presence-only version, and there is still no real hardware timing data for either version.
  **Verified: compile/assemble only.** No change to sweep scope or frequency — still every configured `KeySlot` node, not the full 1-127 range; that wasn't reconsidered as part of this follow-up and shouldn't be assumed safe to narrow without separately checking.

- **Beep-continuity bug investigation (code-only, no hardware available for this investigation itself)** — previously only a working hypothesis, never root-caused. Traced two independent contributing issues in `TerminalKeyTakeScreen.kt`/`TerminalKeyReturnScreen.kt`'s beep-loop `LaunchedEffect`: both were keyed on `beeping` **and** `beepLoud` (Return's also on `wrongSlotNodeAddress`), so the exact moment the 5s-escalation callback flips `beepLoud` and fires the voice line together, Compose cancelled and relaunched the beep coroutine — real, but likely only jitter, not full stoppage. Fixed by keying on `beeping` alone and reading `beepLoud`/`wrongSlotNodeAddress` via `rememberUpdatedState`, identically in both screens. Separately, and judged the more likely actual root cause of the "dies and never resumes" symptom: `AudioFeedbackController`'s beep (`SoundPool` at the time) and every voice line (`MediaPlayer`) shared one `AudioAttributes` instance — on this device's confirmed non-stock `awplayer`/CedarX HAL, a HAL that groups/preempts streams sharing usage/content-type could plausibly kill the beep stream the moment a voice line with matching attributes starts. Fixed by splitting into `beepAudioAttributes` (`USAGE_NOTIFICATION_EVENT`/`CONTENT_TYPE_SONIFICATION`, unchanged) and `voiceLineAudioAttributes` (`USAGE_ASSISTANCE_ACCESSIBILITY`/`CONTENT_TYPE_SPEECH`, new) — this separation is preserved through the later `SoundPool`→`MediaPlayer` beep switch below. Also audited `MAX_CONCURRENT_BEEPS=2` (a question raised alongside these fixes): judged not a plausible contributing factor, since even worst-case coroutine-relaunch jitter only ever produces 2 concurrent streams, which the cap already accommodated without eviction — moot now that beep no longer uses `SoundPool` at all.
  **Verified: compile/assemble only** (`:terminalApp:compileDebugKotlin`, `:terminalApp:assembleDebug`). See the sound-test-tooling entry below for what *was* later hardware-tested, and its explicit caveat that it did **not** reproduce the original concurrent beep+voice-line scenario — see Known issues, still open.

- **Boot auto-launch diagnosis: a real reboot test was run and the app did NOT auto-launch** (the app had already been manually opened at least once before the test, ruling out Android's "stopped app" `BOOT_COMPLETED` exemption, which only applies to apps never launched since install). Re-verified `BootCompletedReceiver`/`AndroidManifest.xml` line by line rather than assuming correctness: the intent-filter action is the exact `android.intent.action.BOOT_COMPLETED` string (no typo); `android:enabled="true"` and `android:exported="true"` are both explicit (not a silent `exported=false`, which would compile fine while quietly blocking the system from invoking the receiver); `RECEIVE_BOOT_COMPLETED` is a correctly top-level `<uses-permission>`, not nested under `<application>`; `onReceive()`'s action check uses the right constant and isn't inverted; the launch `Intent` carries `FLAG_ACTIVITY_NEW_TASK`; and the target class/package (`com.ekms.terminal.MainActivity`) was independently cross-checked against the module's `namespace`/`applicationId` and the actual file's package declaration, not just assumed to match. **No bug was found in any of this.** Added unconditional diagnostic logging (tag `BootAutoLaunch`, no debug-flag gate) at four points: `onReceive()` entry (proves the system invoked the receiver at all), the action-mismatch branch, immediately before `startActivity()`, and a try/catch around `startActivity()` logging any exception (e.g. `ActivityNotFoundException`/`SecurityException`) — so the next reboot test's logcat pinpoints exactly which stage fails instead of staying a black box. Deliberately did **not** build any OEM/vendor "grant autostart permission" workaround — the user is separately checking whether such a device setting even exists on this hardware; guessing at that without data was explicitly out of scope.
  **Verified: compile/assemble only.** The logging is in place but has not yet been exercised by an actual reboot — see Known issues/Next steps.

- **Sound file verification tooling (temporary, built and fully removed within this session) + beep switched from `SoundPool` to `MediaPlayer`.** The device was mid-pairing-gate (pairing-code backend not deployed, see status-truth table) and unreachable through the normal Admin Menu, so verifying newly-dropped-in `res/raw/*.wav` files needed a path independent of login/pairing. Built two throwaway pieces, both explicitly marked "TEMPORARY DEV TOOL — DELETE" in code from the start: `TerminalSoundTestScreen` (an Admin Menu-gated Compose screen, one button per sound) and `SoundTestActivity` (a standalone `ComponentActivity`, `exported="true"`, launched by component name over adb — `adb shell am start -n com.ekms.terminal/.SoundTestActivity --es sound <NAME>` — deliberately bypassing the pairing gate entirely since it's a separate Activity that never touches `TerminalAdminApp`'s Compose tree). Both called the exact same `AudioFeedbackController` methods the real flows use, so this exercised the real playback path, not a mock.
  Used this to build/install/launch on the connected F7G18P (device model `A133P`) and capture filtered logcat (`adb logcat -s SoundTestActivity AudioFeedbackController`) across several runs, including a 15-repetition/1-second-interval beep loop matching the real Take/Return Flow's repeat cadence — all reps and all three voice lines completed cleanly with no drops or errors, across three separate runs (one of which required an explicit `adb shell am force-stop` first, since a still-resident prior `SoundTestActivity` instance silently absorbed a repeat `am start` via its top-of-task rather than restarting — a quirk of this throwaway tool never calling `finish()`, not a production concern). **Caveat: this is a genuine hardware run of the repeat mechanism itself, but it never reproduced the original concurrent scenario** (beep loop still running while a voice line starts at the 5s mark) — `SoundTestActivity`'s sequence runs the beep loop to completion, then plays voice lines separately, so it does **not** hardware-reverify the beep-continuity bug or its fixes above.
  Along the way, found and fixed one real bug surfaced by the automation itself: the very first beep of a fresh `AudioFeedbackController` could silently no-op with zero log output, because `SoundPool.load()` is asynchronous and a beep fired ~280ms after construction could race ahead of `beepLoaded` — added a `Log.w` for the skip case (this was still on `SoundPool` at the time; moot after the engine switch below, but the fix stayed since silent no-ops are worth logging regardless).
  Then, **at the user's explicit request, after being told this would not fix any audible distortion**, switched `beep()` from `SoundPool` to a one-shot `MediaPlayer` (`beepPlayer`), matching `playVoiceLine()`'s construction pattern exactly (`setAudioAttributes()` before `setDataSource()`/`prepareAsync()`). Kept `beepPlayer`/`voiceLinePlayer` as fully separate instances with the distinct `beepAudioAttributes`/`voiceLineAudioAttributes` from the fix above — switching engines doesn't undo that fix. Real, permanent tradeoff: `MediaPlayer` prepares fresh every call (~30-50ms observed per beep in logcat) where `SoundPool` was zero-latency after its one-time load; judged imperceptible for a 1-second repeat cadence, but a genuine behavior change, not just a relabeling.
